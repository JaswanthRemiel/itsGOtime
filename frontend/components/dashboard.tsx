...
export function Dashboard() {
      ...
      try {
        ...
        if (statusRes.ok && historyRes.ok) {
          ...
          setData({
            ...
            console.error("Failed to fetch status or history")
        }
      } catch (e) {
  ...
  const uptimePercentage = totalCount > 0 ? ((onlineCount / totalCount) * 100).toFixed(1) : "0.0"
      ...
      <main className="flex-1 space-y-4 p-4 md:p-8 pt-6">
        <div className="flex flex-col space-y-2">
          ...
          <div className="flex items-center space-x-2 text-muted-foreground">
            ...
            <span className="text-sm">Last updated: {loading ? "Updating..." : data?.generated_at}</span>
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Active services monitoring</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Services online right now</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Average availability across services</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Global latency average</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs defaultValue="all" className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            {searchFilter && (
              <Button variant="ghost" size="sm" onClick={() => setSearchFilter(null)} className="h-8 px-2 lg:px-3">
                Filtered by: <span className="font-bold ml-1">{searchFilter}</span>
        ...
        <CommandList>
          ...
          <CommandGroup heading="Monitors">
            {data?.results.map((monitor) => (
              ...
              >
                <div className={`mr-2 h-2 w-2 rounded-full ${monitor.up ? "bg-emerald-500" : "bg-red-500"}`} />
                <span>{monitor.name}</span>
                <span className="ml-2 text-xs text-muted-foreground font-mono">{monitor.url}</span>
              </CommandItem>
            ))}
          </CommandGroup>
        </CommandList>
      ...
      <div className="flex h-[400px] flex-col items-center justify-center rounded-lg border bg-card text-card-foreground shadow-sm border-dashed">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">
            {searchFilter ? `No monitors matching "${searchFilter}"` : "There are no monitors matching this filter."}
          </p>
        </div>
      </div>
    )
      ...
      <Card>
        <CardHeader className="px-6 py-4 border-b">
          <div className="grid grid-cols-12 gap-4 text-sm font-medium text-muted-foreground uppercase tracking-wider">
            ...
            <div className="col-span-3 md:col-span-4 hidden md:block">Endpoint</div>
          ...
          {filtered.map((monitor, i) => (
            ...
            >
              <div className="col-span-4 md:col-span-3 font-medium flex items-center gap-2 overflow-hidden">
                <div
                  className={`h-2.5 w-2.5 rounded-full shrink-0 ${monitor.up ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]" : "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.4)]"}`}
                />
              ...
              <div className="col-span-4 md:col-span-2 flex justify-end md:justify-start">
                <Badge
                  ...
                  className={
                    monitor.up ? "border-emerald-500/30 text-emerald-500 bg-emerald-500/10 hover:bg-emerald-500/20" : ""
                  }
                >
                  {monitor.up ? "Online" : "Offline"}
                  {!monitor.up && monitor.status !== 0 ? ` (${monitor.status})` : ""}
                </Badge>
              </div>

              <div className="col-span-4 md:col-span-3 h-[40px] flex items-center justify-end">
                <div className="w-full max-w-[160px] h-full">
                  <Sparkline data={history[monitor.name] || []} color={monitor.up ? "#10b981" : "#ef4444"} />
                </div>
              </div>
            </div>
          ))}
